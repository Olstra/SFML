...
int main() {
    ...
    "CLICK ON A BOX TO SELECT IT", "PLAY AGAIN", "select game mode:", "VS COMPUTER", "PLAY WITH FRIEND", "Tic Tac Toe"};
    ...
    // "PLAY AGAIN" button
    sf::RectangleShape playAgainButton(Vector2f(200, 200));
    //sf::Rectangle
    textes[6].setCharacterSize(fontSize/2);

    playAgainButton.setSize(Vector2f( 100, 100));
    //playAgainButton.
    //playAgainButton.setSi
    tempRect = playAgainButton.getLocalBounds();
    playAgainButton.setOrigin(tempRect.left+tempRect.width/2.0f, tempRect.top+tempRect.height/2.0f);

    tempRect = textes[6].getLocalBounds();
    textes[6].setOrigin(tempRect.left+tempRect.width/2.0f, tempRect.top+tempRect.height/2.0f);
    textes[6].setPosition(windowW/2, windowH/15*14);
    textes[6].setFillColor(Color::Black);
    textes[6].setOutlineColor(Color::Yellow);
    textes[6].setOutlineThickness(5.0);

    playAgainButton.setFillColor(Color::Yellow);
    ...
    while (window.isOpen()) {
                    ...
                    if (Mouse::isButtonPressed(Mouse::Left)) {
                        switch (currentState) {
                            ...
                            if(playAgainButton.getGlobalBounds().contains(mousePos.x, mousePos.y)) {
                                if(currentState != mainPlay) { init(); std::cout << "You clicked PLAY AGAIN" << std::endl; }
                            }
                            break;
        ...
        if(currentState != gameMenu && currentState != mainPlay) {
            if(playAgainButton.getFillColor() == Color::Yellow) {
                playAgainButton.setFillColor(Color::White);
            }
            else { playAgainButton.setFillColor(Color::Yellow); }
